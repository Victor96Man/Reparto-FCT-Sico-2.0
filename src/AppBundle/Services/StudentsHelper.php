...
class StudentsHelper
{
    ...
    public function getStudentsDistribution($convocatory){
        ...
        foreach ($students as $student){
            ...
            $student->fct_tutor = (empty($dsc[0]) ? 'exento' : $dsc[0]->getUser()->__toString());
            $student->fct_company = (empty($dsc[0]) ? 'exento': $dsc[0]->getCompany()->__toString());
            $student->fct_id = (empty($dsc[0]) ? 'exento': $dsc[0]->getId());

            $dsp = $distributionProject->getAllForStudent($student->getId());
            $student->pi_tutor = (empty($dsp[0]) ? 'exento' : $dsp[0]->getUser()->__toString());
            $student->pi_project = (empty($dsp[0]) ? 'exento' : $dsp[0]->getProject()->__toString());
            $student->pi_id = (empty($dsp[0]) ? 'exento' : $dsp[0]->getId());
        }
        return $students;
    }
